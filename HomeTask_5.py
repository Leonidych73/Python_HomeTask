...
for car, details in car_data.items():
    color, year, volume, body_type, price = details
    if year >= search_criteria[0] and volume >= search_criteria[1] and price < search_criteria[2]:
        selected_cars[car] = details
sorted_selected_cars = sorted(selected_cars.items(), key=lambda x: x[1][4])  # sort by price

enum_cars = enumerate(sorted_selected_cars)  # make sorted_selected_cars enumerate list for
# printing first five elements
for index, value in enum_cars:
    if index < 5:
        print(value)
    else:
        break
# ______________control_1____________________
